...
using masked autoencoders (MAE). MISFIT allows researchers to pretrain, evaluate,
and deploy large-scale encoder models on unlabeled NIfTI datasets, and to extract
rich semantic embeddings for downstream tasks such as classification, retrieval,
...
These embeddings can be used directly for zero-shot similarity search or fine-tuned
