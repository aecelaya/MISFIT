...
Ampere or newer GPUs (A100, H100, RTX 30xx+). On pre-Ampere cards (T4, V100,
...
requested AMP setting against the current hardware so training, evaluation, and
inference transparently fall back to FP32 instead of silently running a slow or
unsupported path.
...
def resolve_amp(requested: bool, *, warn: bool = True) -> bool:
    ...
    or the GPU is pre-Ampere — a ``UserWarning`` is emitted so callers know
    FP32 will be used instead.
